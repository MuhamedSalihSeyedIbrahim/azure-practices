...
import { info, logger, error } from './logger';
...
export class CosmosDB {
    ...
    async createContainer({ databaseId, containerId, partitionKey, throughPut }) {
        const { container } = await this._dbClient
            .database(databaseId)
            .containers.createIfNotExists({ id: containerId, partitionKey, uniqueKeyPolicy: {
                uniqueKeys: [
                    { paths: ['/poLineItem/poLineItemNumber','/poHeader/poNumber'] }
                ]
