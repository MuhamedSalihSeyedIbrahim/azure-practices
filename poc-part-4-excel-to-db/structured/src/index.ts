...
if(!writeTofile) return
    ...
    if (writeTofile) {
        appController
            .excelToFile()
            .then((_: any) =>
                info(`Application Excel to File Process Data: ${(Date.now() - appStartTime) / (1000 * 60)} min.`),
            )
            .catch((_err: Error) => {
                error(`Application Excel to File has an Error : ${(Date.now() - appStartTime) / (1000 * 60)} min.`);
            })
            .finally(() => {
                console.timeEnd();
                info(`Application Excel to File Stopped : ${(Date.now() - appStartTime) / (1000 * 60)} min.`);
            });
        return;
    }
